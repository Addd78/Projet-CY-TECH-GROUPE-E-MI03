...
// ─────────────────────────────────────────────
//
// Format d'une ligne dans utilisateurs.txt :
//   login:motdepasse:role
//
// Exemple :
//   alice:1234:0
//   prof_martin:azerty:1
//
...
int charger_utilisateurs(Utilisateur tableau[], int taille_max) {
    ...
    // Lire ligne par ligne tant qu'il y a des donnees et qu'on n'a pas
    // depasse la taille maximale du tableau
    while (nb < taille_max &&
           fscanf(f, "%63[^:]:%63[^:]:%d\n",
                  tableau[nb].login,
                  tableau[nb].mdp,
    ...
    fclose(f);
    return nb;   // on retourne le nombre d'utilisateurs charges
}
...
// ─────────────────────────────────────────────
//
// Parcourt le tableau des utilisateurs et compare
// le login ET le mot de passe saisis.
// Si une correspondance est trouvee, copie l'utilisateur
// dans user_connecte et retourne 1.
